...
class WebUISettingsRouter:
    ...
    async def dispatch(self, connection: Any, request: WsRequest, path: str) -> Response | None:
        ...
        if path == "/api/settings/video-generation/update":
            return await self._handle_settings_video_generation_update(request)
        ...
        if path == "/api/settings/storage/update":
            return self._handle_settings_storage_update(request)
    ...
    async def _handle_settings_video_generation_update(self, request: WsRequest) -> Response:
        if not self._authorized(request):
            return self._unauthorized()
        try:
    ...
    def _handle_settings_storage_update(self, request: WsRequest) -> Response:
        if not self._authorized(request):
            return self._unauthorized()
        try:
            payload = update_storage_settings(self._query(request))
        except WebUISettingsError as e:
            return self._error_response(e.status, e.message)
        return self._json_response(self._with_restart_state(payload, section="runtime"))
